...
download = BashOperator(
    ...
    bash_command="cd /home/airflow/gcs/data/pub_metadata_update && wget -N 'ftp://ftp.ncbi.nlm.nih.gov:21/pubmed/updatefiles/pubmed23n*' 2>&1  | grep done | grep '.gz' | grep -v '.gz.md5' | tr -s ' ' | cut -f 7 -d ' '  > pub-metadata-update-downloaded-files.txt",
    dag=dag)
...
verify_md5 = BashOperator(
    ...
    bash_command="cd /home/airflow/gcs/data/pub_metadata_update && md5sum --check *.gz.md5",
    dag=dag)

populate_load_directory = BashOperator(
    task_id='populate-load-directory',
    bash_command="cd /home/airflow/gcs/data/pub_metadata_update && mkdir -p /home/airflow/gcs/data/pub_metadata_update/to_load && if [ -s pub-metadata-update-downloaded-files.txt ]; then xargs -a pub-metadata-update-downloaded-files.txt cp -t /home/airflow/gcs/data/pub_metadata_update/to_load; fi",
    dag=dag)
...
def wrap_title_and_abstract_with_cdata():
    ...
    for root, dirs, files in os.walk(os.path.abspath("/home/airflow/gcs/data/pub_metadata_update/to_load")):
        for f in files:
...
def check_for_files_to_process(**kwargs):
    dir = os.listdir("/home/airflow/gcs/data/pub_metadata_update/to_load")
    if len(dir) > 0:
...
create_publication_metadata = BashOperator(
    ...
    bash_command="mkdir -p /home/airflow/gcs/data/pub_metadata_update/metadata && ls -lhrt /home/airflow/gcs/data && jar -tf /home/airflow/gcs/data/$TM_PIPELINES_JAR_NAME | grep MedlineUiMetadataExtractor && java -cp /home/airflow/gcs/data/$TM_PIPELINES_JAR_NAME edu.cuanschutz.ccp.tm_provider.corpora.MedlineUiMetadataExtractor /home/airflow/gcs/data/pub_metadata_update/to_load /home/airflow/gcs/data/pub_metadata_update/metadata 0",
...
aggregate_publication_metadata = BashOperator(
    ...
    bash_command="gunzip -c /home/airflow/gcs/data/pub_metadata_update/metadata/*.ui_metadata.tsv.gz | grep DOC_ID | sort | uniq > /home/airflow/gcs/data/pub_metadata_update/metadata/{{ ts_nodash }}.ui_metadata.tsv && gunzip -c /home/airflow/gcs/data/pub_metadata_update/metadata/*.ui_metadata.tsv.gz | grep PMID >> /home/airflow/gcs/data/pub_metadata_update/metadata/{{ ts_nodash }}.ui_metadata.tsv && gzip /home/airflow/gcs/data/pub_metadata_update/metadata/{{ ts_nodash }}.ui_metadata.tsv && gunzip -c /home/airflow/gcs/data/pub_metadata_update/metadata/*.ui_metadata.delete.tsv.gz > /home/airflow/gcs/data/pub_metadata_update/metadata/{{ ts_nodash }}.ui_metadata.delete.tsv && gzip /home/airflow/gcs/data/pub_metadata_update/metadata/{{ ts_nodash }}.ui_metadata.delete.tsv",
    dag=dag)
...
create_processed_pmids_file = BashOperator(
    ...
    bash_command="gunzip -c /home/airflow/gcs/data/pub_metadata_update/metadata/{{ ts_nodash }}.ui_metadata.tsv.gz | cut -f 1 | grep PMID > /home/airflow/gcs/data/pub_metadata_update/metadata/processed.pmids",
    dag=dag)
...
move_aggregate_publication_data_to_bucket = BashOperator(
    ...
    bash_command="CLOUDSDK_PYTHON=/usr/bin/python gsutil cp /home/airflow/gcs/data/pub_metadata_update/metadata/{{ ts_nodash }}.ui_metadata.tsv.gz gs://$PUBLICATION_METADATA_BUCKET/$PUBLICATION_METADATA_PATH && CLOUDSDK_PYTHON=/usr/bin/python gsutil cp /home/airflow/gcs/data/pub_metadata_update/metadata/{{ ts_nodash }}.ui_metadata.delete.tsv.gz gs://$PUBLICATION_METADATA_BUCKET/$PUBLICATION_METADATA_PATH",
...
pipeline_end = BashOperator(
    ...
    bash_command="rm /home/airflow/gcs/data/pub_metadata_update/to_load/* && rm /home/airflow/gcs/data/pub_metadata_update/metadata/*",
    dag=dag)
...
dataflow_medline_xml_to_text.set_upstream(trigger_publication_metadata_delete)
update_pubmed_ids_catalog.set_upstream(dataflow_medline_xml_to_text)
pipeline_end.set_upstream(update_pubmed_ids_catalog)
