...
download = BashOperator(
    ...
    bash_command="cd /home/airflow/gcs/data/medline_load && wget -N 'ftp://ftp.ncbi.nlm.nih.gov:21/pubmed/updatefiles/pubmed23n*' 2>&1  | grep done | grep '.gz' | grep -v '.gz.md5' | tr -s ' ' | cut -f 7 -d ' '  > downloaded-files.txt",
    dag=dag)
...
verify_md5 = BashOperator(
    ...
    bash_command="cd /home/airflow/gcs/data/medline_load && md5sum --check *.gz.md5",
    dag=dag)

populate_load_directory = BashOperator(
    task_id='populate-load-directory',
    bash_command="cd /home/airflow/gcs/data/medline_load && mkdir -p /home/airflow/gcs/data/medline_load/to_load && if [ -s downloaded-files.txt ]; then xargs -a downloaded-files.txt cp -t /home/airflow/gcs/data/medline_load/to_load; fi",
    dag=dag)
...
pipeline_end = BashOperator(
    ...
    bash_command="rm /home/airflow/gcs/data/medline_load/to_load/* && rm /home/airflow/gcs/data/medline_load/metadata/*",
    dag=dag)
