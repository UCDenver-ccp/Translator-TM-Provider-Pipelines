...
public class JatsArticleToDocumentFn extends DoFn<Article, KV<String, List<String>>> {
	...
	public JatsArticleToDocumentFn(com.google.cloud.Timestamp timestamp,
			Map<String, DocumentCriteria> outputDocumentCriteria, PCollectionView<Set<String>> currentDocIDs) {
	...
	public static TextDocumentWithMetadata buildDocument(Article article) {
		...
		if (body != null && !body.toString().trim().isEmpty()) {
			...
			// Iterate the paragraph-level annotations (this assumes they are in document
			// order)
	...
	private static String getAbstractText(ArticleMeta meta) {
		...
		// Depending on how it works with multiple Abstract sections this may need a
		// newline separator.
