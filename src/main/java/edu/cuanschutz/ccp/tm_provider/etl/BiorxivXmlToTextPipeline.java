...
public class BiorxivXmlToTextPipeline {
    ...
    public static void main(String[] args) {
        ...
        MedlineXmlToTextPipeline.Options options = PipelineOptionsFactory.fromArgs(args).withValidation().as(MedlineXmlToTextPipeline.Options.class);
        ...
        DocumentCriteria sectionCriteria = new DocumentCriteria(DocumentType.SECTIONS, DocumentFormat.BIONLP, PipelineKey.BIORXIV_XML_TO_TEXT, pipelineVersion);
        Map<String, DocumentCriteria> criteriaMap = Collections.singletonMap("sections", sectionCriteria);

        PCollectionTuple outputs = articles.apply(ParDo.of(new JatsArticleToDocumentFn(timestamp, criteriaMap, existingDocIds))
                .withSideInputs(existingDocIds)
        ...
        PCollection<KV<String, List<String>>> nonredundantSectionAnnotations = PipelineMain.deduplicateDocumentsByStringKey(sections);
        // Convert Section Annotation to Entity, and Save to DataStore
        nonredundantSectionAnnotations
                .apply("section annotation -> annotation entity",
                        ParDo.of(new JatsDocumentToEntityFn(sectionCriteria, collectionsMap))
                                .withSideInputs(collectionsMap))
                .apply("section annotation entity -> datastore",  DatastoreIO.v1().write().withProjectId(options.getProject()));
        //endregion
