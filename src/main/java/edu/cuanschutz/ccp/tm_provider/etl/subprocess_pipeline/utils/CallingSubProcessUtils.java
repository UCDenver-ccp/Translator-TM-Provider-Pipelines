 ...
 */
package edu.cuanschutz.ccp.tm_provider.etl.subprocess_pipeline.utils;
 ...
 * 
 * WAB - Modified from:
 * https://github.com/apache/beam/blob/master/examples/java/src/main/java/org/apache/beam/examples/subprocess/utils/CallingSubProcessUtils.java
 * <br>
 * to: <br>
 * 1) handle a list of files that need to be copied from a GCP bucket to the
 * worker
 */
public class CallingSubProcessUtils {
	...
	// Allow multiple subclasses to create files, but only one thread per subclass
	// can add the file to
	// the worker
	...
	public static void setUp(SubProcessConfiguration configuration, List<String> filesToDownload) throws Exception {
		...
		for (String binaryName : filesToDownload) {
