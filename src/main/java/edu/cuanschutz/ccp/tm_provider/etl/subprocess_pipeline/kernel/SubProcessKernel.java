 ...
 */
package edu.cuanschutz.ccp.tm_provider.etl.subprocess_pipeline.kernel;
...
/**
 * This is the process kernel which deals with exec of the subprocess. It also
 * deals with all I/O.
 * 
 * 
 * 
 * WAB - Modified from:
 * https://github.com/apache/beam/blob/master/examples/java/src/main/java/org/apache/beam/examples/subprocess/kernel/SubProcessKernel.java
 * <br>
 * to: <br>
 * 1) set the working directory for the process to be the same directory where
 * the binary exists; this was necessary b/c the Ab3P system looks for a file in
 * the working directory to tell it where to look for other resource files. <br>
 * <br>
 * 2) remove the results file from the command - Ab3P outputs to standard out
 * and we capture the output from standard out not a results file. This may be
 * non-ideal and perhaps changed in the future but that's how it works
 * currently. Changes were required because an exception was previously thrown
 * if the results file didn't exist <br>
 * <br>
 * 3) replaced direct access of variables with getters for the
 * SubProcessCommandLineArgs.Command and SubProcessIOFiles classes
 */
...
public class SubProcessKernel {
	...
	private File binaryWorkingDirectory;

	private SubProcessKernel() {
	}

	/**
	 * Creates the SubProcess Kernel ready for execution. Will deal with all input
	 * and outputs to the SubProcess
	 *
	 * @param options
	 * @param binaryName
	 */
	public SubProcessKernel(SubProcessConfiguration options, String binaryName, File binaryWorkingDirectory) {
		this.configuration = options;
		this.binaryWorkingDirectory = binaryWorkingDirectory;
	...
	public List<String> exec(SubProcessCommandLineArgs commands) throws Exception {
		try (CallingSubProcessUtils.Permit permit = new CallingSubProcessUtils.Permit(
			...
			try (SubProcessIOFiles outputFiles = new SubProcessIOFiles(configuration.getWorkerPath())) {
				...
				LOG.error("Unable to delete the outputfiles. This can lead to performance issues and failure", ex);
			}
			return results;
		}
	}
	...
	private ProcessBuilder prepareBuilder(ProcessBuilder builder, SubProcessCommandLineArgs commands,
		...
		// Add the result file path to the builder at position 1, 0 is reserved for the
		// process itself
//    builder.command().add(1, outPutFiles.resultFile.toString());
		...
		for (SubProcessCommandLineArgs.Command s : commands.getParameters()) {
			builder.command().add(s.getKey() + 1, s.getValue());
		}

		builder.redirectError(Redirect.appendTo(outPutFiles.getErrFile().toFile()));
		builder.redirectOutput(Redirect.appendTo(outPutFiles.getOutFile().toFile()));

//		for (int i = 0; i < builder.command().size(); i++) {
//			String command = builder.command().get(i);
//			LOG.warn("BUILDER COMMANDS (" + i + "): " + command);
//		}
//
//		File f = new File(configuration.getWorkerPath());
//		for (File ff : f.listFiles()) {
//			LOG.warn("DIRECTORY CONTENTS: " + ff.getAbsolutePath());
//			if (ff.isDirectory()) {
//				for (File fff : ff.listFiles()) {
//					LOG.warn("SUBDIRECTORY CONTENTS: " + fff.getAbsolutePath());
//				}
//			}
//		}
	...
	private Process execBinary(ProcessBuilder builder, SubProcessCommandLineArgs commands,
		...
		try {
			...
			builder.directory(binaryWorkingDirectory);
	 ...
	 *                   exception
	 */
	private List<String> collectProcessResults(Process process, ProcessBuilder builder, SubProcessIOFiles outPutFiles)
			throws Exception {
		...
		try {
			...
			// TODO: there is no output file created - output is sent to standard out -
			// perhaps this can be adjusted at some point, but we won't error if the result
			// file does not exist, b/c it will never exist.
//			// If no return file then either something went wrong or the binary is setup
//			// incorrectly for
...
//			}
			...
			// Everything looks healthy return values -- return from out file instead of
			// results file
//			try (Stream<String> lines = Files.lines(outPutFiles.resultFile)) {
			try (Stream<String> lines = Files.lines(outPutFiles.getOutFile())) {
	 ...
	 *                   exception
	 */
	private byte[] collectProcessResultsBytes(Process process, ProcessBuilder builder, SubProcessIOFiles outPutFiles)
			throws Exception {
...
//			}
			...
			// Everything looks healthy return bytes -- return from out file instead of
			// results file
//			return Files.readAllBytes(outPutFiles.resultFile);
			return Files.readAllBytes(outPutFiles.getOutFile());
	...
	private static String createLogEntryForProcessFailure(Process process, List<String> commands,
		...
		stringBuilder.append(String.format("Command info was %s %n", createLogEntryFromInputs(commands)));

		stringBuilder.append(
				String.format("First line of error file is  %s %n", FileUtils.readLineOfLogFile(files.getErrFile())));

		stringBuilder.append(
				String.format("First line of out file is %s %n", FileUtils.readLineOfLogFile(files.getOutFile())));

		stringBuilder.append(
				String.format("First line of ret file is %s %n", FileUtils.readLineOfLogFile(files.getResultFile())));
	...
	private ProcessBuilder appendExecutablePath(ProcessBuilder builder) {
		...
		builder.command().set(0, FileUtils.getFileResourceId(configuration.getWorkerPath(), executable).toString());
		return builder;
	}
}