...
class BioCReader implements Closeable {
	...
	protected Object read() throws XMLStreamException {
...
//          DTD2 dtd = (DTD2) event;
			...
			case 1:
				if (event.isStartElement()) {
					...
					if (localName.equals("source")) {
						...
						collection.putInfon(getAttribute(startElement, "key"), getText());
			...
			case 2:
				if (event.isStartElement()) {
					...
					if (localName.equals("id")) {
						...
						document.putInfon(getAttribute(startElement, "key"), getText());
			...
			case 3:
				if (event.isStartElement()) {
					...
					if (localName.equals("offset")) {
						...
						passage.putInfon(getAttribute(startElement, "key"), getText());
					} else if (localName.equals("annotation")) {
			...
			case 4:
				if (event.isStartElement()) {
					...
					if (localName.equals("offset")) {
						...
						sentence.putInfon(getAttribute(startElement, "key"), getText());
					} else if (localName.equals("annotation")) {
	...
	private BioCAnnotation readAnnotation(StartElement annotationEvent) throws XMLStreamException {
		...
		while (reader.hasNext()) {
			...
			if (event.isStartElement()) {
				...
				if (localName.equals("text")) {
					...
					ann.putInfon(startElement.getAttributeByName(new QName("key")).getValue(), getText());
				} else if (localName.equals("location")) {
					ann.addLocation(new BioCLocation(Integer.parseInt(getAttribute(startElement, "offset")),
	...
	private BioCRelation readRelation(StartElement relationEvent) throws XMLStreamException {
		...
		while (reader.hasNext()) {
			...
			if (event.isStartElement()) {
				...
				if (localName.equals("infon")) {
					rel.putInfon(getAttribute(startElement, "key"), getText());
